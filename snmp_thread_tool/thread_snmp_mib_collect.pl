...
##---------------------------------------------------------------------------
#
# FILENAME		: thread_snmp_mib_collect.pl
# PROGRAM		: perl program
# CREATE DATE	: 2009/10/20
# MODIFY DATE	: 
# AUTHOR		: Ning Bin
# USAGE			: thread_snmp_mib_collect.pl 
...
{
	$node_conf_queue = new Thread::Queue;
	while(<NODE_CONF>)
	{
		chomp;
		$trace->($_);
		...
		next if( /^\s?$/ );				#null line
		s/ //g;							#remove space
		my @conf_list :shared = split /,/ ;		#devide line by ','
		
		$node_conf_queue->enqueue( \@conf_list );
		
		push( @node_conf_list, \@conf_list );
	}
	$node_conf_queue->enqueue( (undef) x $THREADS );
	close NODE_CONF;
}
...
sub CollectMib
{
	my @thread_pool = map{ threads->create( \&CollectThread ) } 1 .. $THREADS;
...
sub CollectThread
{
	...
	while( my $node = $node_conf_queue->dequeue )
	{
       &DoOneSession( $node );
    }
...
sub DoOneSession
{
	...
	my $node = ();
	$node->{ip}=$$node_array[0];
	$node->{community}=$$node_array[1];
	$node->{port}=$$node_array[2];
	$node->{hostname}=$$node_array[3];
...
{
	my $node = shift;
	$trace->("select HOST_NAME from $TABLE_NAME where IP_ADD = '$$node[0]'");
	
	$sel_session->bind_param( 1, $$node[0] );
	$sel_session->execute();
